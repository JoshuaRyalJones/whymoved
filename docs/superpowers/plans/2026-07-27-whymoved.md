...
```bash
git add -A
git commit -m "chore: scaffold Next.js app with Vitest and attribution types"
```
...
git commit -m "feat: add return and contribution math with exact summation"
```
...
git commit -m "feat: add OLS beta estimation with low-confidence fallback"
```
...
**Files:**
- Create: `src/lib/attribution/classify.ts`
- Test: `tests/attribution/classify.test.ts`

**Interfaces:**
- Consumes: `Classification`, `MoveLabel` from `~/lib/attribution/types`
- Produces: `classifyMove(params: ClassifyParams): Classification`, and the exported `ClassifyParams` interface
...
```bash
git add src/lib/attribution/classify.ts tests/attribution/classify.test.ts
git commit -m "feat: add move classification gating LLM calls on residual z-score"
```
...
git commit -m "feat: add PortfolioSource interface with manual and demo sources"
```
...
git commit -m "feat: add Wealthsimple holdings CSV import with multi-account aggregation"
```
...
git commit -m "feat: add Tiingo EOD price client with injectable fetch"
```
...
git commit -m "feat: add Finnhub company news client with local article IDs"
```
...
git commit -m "feat: add hard citation validation with no_driver fallback"
```
...
git commit -m "feat: add Claude explanation call with forced tool schema and citation retry"
```
...
git commit -m "feat: add database schema with RLS and service client"
```
...
**Interfaces:**
- Consumes: `computeContributions`, `portfolioReturn` (Task 2); `computeBeta` (Task 3); `classifyMove` (Task 4); `PortfolioSource`, `Holding` (Task 5); `DailyPrice`, `toDailyReturns` (Task 7); `Article` (Task 8); `explainMove` (Task 10)
- Produces:
  - `interface HoldingResult { ticker: string; contribution: Contribution; classification: Classification; explanation: Explanation | null }`
...
// Market moves +/-1% daily. Both holdings track it with small idiosyncratic noise
// bounded at +/-0.2%, which caps their z-score below the 2.0 threshold by construction:
// for a uniform distribution the maximum deviation is sqrt(3) sigma, i.e. ~1.73.
const marketReturns = Array.from({ length: N }, () => (rand() - 0.5) * 0.02)
const aaaReturns = marketReturns.map((m) => m + (rand() - 0.5) * 0.004)
const bbbReturns = marketReturns.map((m) => m + (rand() - 0.5) * 0.004)

// BBB alone takes a large idiosyncratic jump on the final day.
bbbReturns[N - 1] = marketReturns[N - 1] + 0.25

const marketPrices = pricesFromReturns(marketReturns)
...
function makeDeps(overrides: Partial<Parameters<typeof runDailyPipeline>[0]> = {}) {
  return {
    ...
    holdings,
    benchmarkTicker: 'SPY',
    fetchPrices: vi.fn(async (ticker: string) => {
      if (ticker === 'SPY') return marketPrices
      if (ticker === 'AAA') return aaaPrices
      return bbbPrices
    }),
...
describe('runDailyPipeline', () => {
  ...
  it('skips holdings with insufficient price history', async () => {
    ...
    const result = await runDailyPipeline(deps)
    expect(result.holdings.map((h) => h.ticker)).toEqual(['AAA'])
  })

  it('throws when the benchmark has no price history', async () => {
    const deps = makeDeps({ fetchPrices: vi.fn(async () => [] as DailyPrice[]) })
    await expect(runDailyPipeline(deps)).rejects.toThrow('no price history for benchmark SPY')
  })
})
```
...
export async function runDailyPipeline(params: DailyPipelineParams): Promise<DailyResult> {
  ...
  const benchmarkPrices = await params.fetchPrices(params.benchmarkTicker)
  if (benchmarkPrices.length < 2) {
    throw new Error(`no price history for benchmark ${params.benchmarkTicker}`)
  }
  const marketReturns = toDailyReturns(benchmarkPrices)
  ...
  for (let i = 0; i < priced.length; i++) {
    ...
    results.push({ ticker: contribution.ticker, contribution, classification, explanation })
  }
...
git commit -m "feat: add daily pipeline gating news and LLM calls on classification"
```
...
git commit -m "feat: add authenticated daily cron route with result persistence"
```
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-12 prose dark:prose-invert">
      ...
      <p>
        ...
        least squares over the trailing 252 trading days.
      </p>
...
git commit -m "feat: add dashboard, drill-down, demo, methodology, and explanation rating"
```
...
git commit -m "feat: add SnapTrade connection flow and position sync"
```
...
git commit -m "feat: add eval metrics harness and README"
```
...
- Per-exchange benchmark selection (v1 uses SPY for all holdings; `securities.benchmark_ticker` already exists to support this)
